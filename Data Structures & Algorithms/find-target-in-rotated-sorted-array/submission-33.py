class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        # i came back to optimize it, no longer need a separate chunk to handle when the array is already perfectly sorted. just set low = 0 and high to last index if already sorted
        ...
        # if else statement to check if array is already perfectly sorted
        low = 0
        high = len(nums)-1
        if nums[low] < nums[high]:
            high = len(nums)
